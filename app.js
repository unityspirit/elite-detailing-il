...
function runCounters(pi) {
  ...
  section.querySelectorAll('.counter').forEach(el => {
    ...
    const t = setInterval(() => {
      cur += step;
      if (cur >= tgt) { el.textContent = tgt.toLocaleString('he-IL'); clearInterval(t); return; }
      el.textContent = Math.floor(cur).toLocaleString('he-IL');
    }, 16);
  });
}
  ...
  setTimeout(() => {
    document.querySelector('.contact-form').innerHTML = `
      <div class="form-success" style="text-align:center;padding:40px 20px;">
        ...
        <div style="font-family:var(--font-heading);font-size:1.8rem;color:var(--blue);margin-bottom:8px;letter-spacing:4px;text-shadow:var(--ts);">הפנייה התקבלה!</div>
